...
def arvin_test(request):
    type_obj = Host.objects.all().values('is_delet', 'remark').distinct()
    obj = {"app_id": 2,
...
def search_monitor_host(request):
    ...
    if filter_obj['biz_id']:
        hosts = Host.objects.filter(app_id=filter_obj['biz_id'], ip_address__contains=filter_obj['ip'],
                                    is_deleted=False)
    else:
        hosts = Host.objects.filter(ip_address__contains=filter_obj['ip'], is_deleted=False)
    return_data = [i.toDic() for i in hosts]
    return render_json({'resutl': True, 'data': return_data})


def get_all_monitor_host(request):
...
def add_host_monitor(request):
    try:
        ...
        for obj in host_obj:
            host = Host.objects.filter(app_id=obj['biz_id'], ip_address=obj['ip_address'], cloud_id=obj['cloud_id'])
            if not host:
                host = {}
                host["ip_address"] = obj["ip_address"]
                host["cloud_id"] = obj["cloud_id"]
                host["app_id"] = obj["biz_id"]
                host["cloud_name"] = obj["cloud_name"]
                host["os_name"] = obj["os_name"]
                host["created_by"] = request.user.username
                host["when_created"] = str(datetime.datetime.now())
...
def rm_host_monitor(request):
    ...
    Host.objects.filter(app_id=obj['app_id'], ip_address=obj['ip_address'], cloud_id=obj['cloud_id']).update(
        is_deleted=True)
    return render_json({'result': True, 'data': ''})
...
#         Host.objects.filter(remark=True).update(remark=False)
#         host = Host.objects.get(app_id=obj['biz_id'], ip_address=obj['ip_address'], cloud_id=obj['cloud_id'])
#         host.remark = True
#         host.save()
#     except:
#         host = Host.objects.get(remark=1)
#     try:
...
def get_monitor_detail(request):
    try:
        obj = json.loads(request.body)
        host = Host.objects.get(ip_address=obj['ip_address'])
        detail = MonitorDetail.objects.filter(server=host).last()
        mem = {"used": detail.mem_usage, "unuse": float(100.0-float(detail.mem_usage))}
        disk = detail.disk_usage.split("\n")
        disk_info = []
        for i in disk[1:-1]:
            data = i.strip().split()
            disk_obj = {
        ...
        load_avg = {'x': [], 'y': []}
        date_now = datetime.datetime.now() + datetime.timedelta(hours=-1)
        details = MonitorDetail.objects.filter(server=host,
                                               when_created__gt=str(date_now).split(".")[0]).order_by('id')
        for i in details:
            load_avg['x'].append(i.when_created)
            load_avg['y'].append(i.cpu_usage)
        return render_json({'result': True, 'mem': mem, 'disk_info': disk_info, 'load_avg': load_avg})
    except Exception, e:
        return render_json({'result': False, 'message': e.message})
